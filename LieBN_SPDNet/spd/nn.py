...
class BiMap(nn.Module):
    """
    Input X: (batch_size,hi) SPDDR matrices of size (ni,ni)
    Output P: (batch_size,ho) of bilinearly mapped matrices of size (no,no)
    Stiefel parameter of size (ho,hi,ni,no)
    """
    def __init__(self,ho,hi,ni,no,device = th.device('cpu'),dtype=dtype,optimizer='geoopt'):
        super(BiMap, self).__init__()
        if optimizer == 'geoopt':
            self._W = geoopt.ManifoldParameter(th.empty(ho, hi, ni, no, dtype=dtype, device=device), manifold=geoopt.manifolds.Stiefel())
        elif optimizer == 'homemade':
            self._W=functional.StiefelParameter(th.empty(ho,hi,ni,no,dtype=dtype,device=device))
        else:
            raise Exception('unknown param_mode {}'.format(optimizer))
...
class BaryGeom(nn.Module):
    ...
    Input x is a batch of SPDDR matrices (batch_size,1,n,n) to average
...
class BatchNormSPD(nn.Module):
    ...
    def forward(self,X):
        ...
        # num_x = X.data.numpy()
        # num_X_centered = X_centered.data.numpy()
        # num_mean = mean.data.numpy()
