...
from spdnets.LieBNImpl import SPDLieBatchNormImpl
from spdnets.BaseBatchNorm import BaseBatchNorm,BatchNormDispersion,BatchNormTestStatsMode,BatchNormTestStatsInterface
...
class SPDBatchNorm(SPDLieBatchNormImpl):
    """
...
class SPDBatchReNorm(SPDLieBatchNormImpl):
    """
...
class AdaMomSPDBatchNorm(SPDLieBatchNormImpl, SchedulableBatchNorm):
    """
    ...
    def __init__(self, shape: Tuple[int, ...] or torch.Size,
                 batchdim: int,
                 eta=1.0, eta_test=0.1,
                 metric='AIM',theta=1.,alpha=1.0,beta=0.,**kwargs):
        super().__init__(shape=shape, batchdim=batchdim,
                         eta=eta, eta_test=eta_test,metric=metric,theta=theta,alpha=alpha,beta=beta, **kwargs)
...
class DomainSPDBatchNormImpl(BaseDomainBatchNorm):
    ...
    def __init__(self, shape: Tuple[int, ...] or torch.Size, batchdim: int,
                 ...
                 dispersion: BatchNormDispersion = BatchNormDispersion.NONE,
                 test_stats_mode: BatchNormTestStatsMode = BatchNormTestStatsMode.BUFFER,
                 eta=1., eta_test=0.1, domains: Tensor = Tensor([]),
                 metric='AIM',theta=1.,alpha=1.0,beta=0.,**kwargs):
        super().__init__()
        self.metric=metric;self.theta=theta;self.alpha=alpha;self.beta=beta;
        ...
        for domain in domains:
            self.add_domain_(cls(shape=shape, batchdim=batchdim,
                                 ...
                                 metric=metric,theta=theta,alpha=alpha,beta=beta,**kwargs),
                             domain)
        self.set_test_stats_mode(test_stats_mode)
...
class DomainSPDBatchNorm(DomainSPDBatchNormImpl):
    ...
    domain_bn_cls = SPDLieBatchNormImpl


class AdaMomDomainSPDLieBatchNorm(SchedulableDomainBatchNorm, DomainSPDBatchNormImpl):
    """
    Combines domain-specific batch normalization on the SPD manifold
    with adaptive momentum batch normalization [Yong et al. 2020, ECCV].
